...
T_vals = 1200:2500;
ratioPho = zeros(1, length(T_vals));
...
gRespPho = [0; gResp; 0] * 1.1; % for 2000 K uncalibrated = 1.2198881
...
%     ylabel('Relative Sensitivity')
    ...
    % no lambda
    int_R_pho = @(y) gamma_R_pho(y) .* G_lookup(y);
    int_G_pho = @(y) gamma_G_pho(y) .* G_lookup(y);
    int_B_pho = @(y) gamma_B_pho(y) .* G_lookup(y);
    ...
    ratioPho(i) = S_G_pho/S_R_pho;
    
end

load('GRnoFilterFiberWin1.1.mat', 'grRatio')

ratioPho = ratioPho';

ratioErr = abs(grRatio - ratioPho) ./ grRatio * 100;

plot(T_vals, ratioErr)
title('G/R Ratio Reported vs. Theoretical Percent Error')
